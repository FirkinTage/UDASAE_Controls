...
def get_data():
    ...
    if (cond == True):
        ...
        try:
            ...
            last_lon = float(df['lon'].iloc[-2])
            last_lat = float(df['lat'].iloc[-2]) 
            distance = [lon - last_lon, lat - last_lat] # gives the x and y distance between the current and previous positions
            theta = (180/np.pi)*math.atan2(distance[1], distance[0]) # angle between points
            scatter = ax.scatter(lon, lat, marker = (3, 0, theta), color = 'c')
            canvas.draw()
